...
from .crypto import ExecutionGraph, make_key, public_pem
...
async def pay(state):
    ...
    async with httpx.AsyncClient(timeout=5) as c: r=await c.post(f"{STORE_URL}/payment",json={k:a[k] for k in ("transaction","signature","transaction_public_key","dae_attestation_signature")})
...
async def run(body: Run):
    if body.scenario not in {"standard","injection","toctou","deviation"}: raise HTTPException(400,"unknown scenario")
