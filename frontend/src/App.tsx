...
export default function App() {
  const [result,setResult]=useState<{authorized:boolean;reason?:string;graph:ExecutionNode[];payment?:{status:string}}|null>(null); const [loading,setLoading]=useState(false);
  async function run(){setLoading(true); try { const r=await fetch('/api/run',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({scenario:'standard',budget:25,domain:'mockstore.local'})}); setResult(await r.json()); } finally {setLoading(false)} }
  return <main className="grid-bg min-h-screen px-5 py-6 md:px-10">
    ...
    <section className="mx-auto max-w-7xl pt-20 md:pt-28">
      ...
      <div className="mt-10 grid gap-5 lg:grid-cols-[.8fr_1.2fr]"><section className="panel rounded-2xl p-7"><p className="text-xs tracking-[.18em] text-slate-500">LIVE AUTHORIZATION</p><h2 className="mt-3 text-2xl">Secure VPS acquisition</h2><p className="mt-3 text-sm leading-6 text-slate-400">Intent: acquire a VPS from mockstore.local within a $25 budget.</p><div className="mt-7 flex items-center justify-between rounded-xl border border-slate-700/70 p-4"><span className="text-sm text-slate-400">Spend cap</span><b className="text-2xl">$25</b></div><button onClick={run} disabled={loading} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-200 disabled:opacity-60"><Play size={15}/>{loading?'VERIFYING PATH…':'RUN PROTECTED EXECUTION'}</button>{result&&<p className={result.authorized ? "mt-4 text-sm text-emerald-300" : "mt-4 text-sm text-rose-300"}>{result.authorized?'AUTHORIZED · TRANSACTION SIGNED':'BLOCKED · '+result.reason}</p>}</section><GraphVisualizer nodes={result?.graph??[]} authorized={result?.authorized} reason={result?.reason}/></div>
